...
public class SimpleBinder implements Binder {
    ...
    private final SimpleBindingStack   bindingStack;
    private       DynamicConfiguration dc;

    @Inject
    public SimpleBinder( App app, SimpleBindingStack bindingStack ) {
        this.app = app;
    ...
    @Override
    public <T> Binding<T> bindService( Class<T> implementationClass ) {
        return new BindingBuilderImpl<>( app, bindingStack, dc, implementationClass );
    }

    @Override
    public <T> Binding<T> bindService( T implementation ) {
